...
func printPromptReceiverLocked() { // requer outMu bloqueado
	fmt.Print("Digite o usuário para quem quer enviar uma mensagem: ")
}
...
func main() {
	...
	// 5) Loop de envio interativo
	for {
		// — Pergunta DESTINATÁRIO —
		outMu.Lock()
		curPrompt = pkReceiver
		printPromptReceiverLocked()
		outMu.Unlock()

		receiver, err := reader.ReadString('\n')
		if err != nil {
			log.Printf("erro leitura receiver: %v", err)
			continue
		}
		...
		// Confirma e volta ao prompt de destinatário
		outMu.Lock()
